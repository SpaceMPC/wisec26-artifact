...
protocols = ['semi','hemi','soho','temi','mascot','spdz2k']
...
time_dict_offline = {}
for protocol in protocols:
    time_df = pd.read_csv(f'../final_data/qp-with-direct/{protocol}_time.csv')
    time_dict_online[protocol] = np.mean(np.asarray(time_df['online']))
    time_dict_offline[protocol] = np.mean(np.asarray(time_df[' offline']))
    

data_dict_online = {}
data_dict_offline = {}
for protocol in protocols:
    data_df = pd.read_csv(f'../final_data/qp-with-direct/{protocol}_data.csv')
    data_dict_online[protocol] = np.mean(np.asarray(data_df['online']))
    data_dict_offline[protocol] = np.mean(np.asarray(data_df[' offline']))

rounds_dict_online = {}
rounds_dict_offline = {}
for protocol in protocols:
    rounds_df = pd.read_csv(f'../final_data/qp-with-direct/{protocol}_rounds.csv')
    rounds_dict_online[protocol] = np.mean(np.asarray(rounds_df['online']))
    rounds_dict_offline[protocol] = np.mean(np.asarray(rounds_df[' offline']))

fig, ax = plt.subplots(3, 2, figsize=(20, 12))
text_size_over_labels = -3

#####################################################################################################

#order everything nicely -- TIME -- online
build_graph(time_dict_online, 2, 'Seconds (s)', protocols, sec_guarantee, ax[0][0],text_size_over_labels=text_size_over_labels)

#####################################################################################################

#order everything nicely -- TIME -- offline
build_graph(time_dict_offline, 2, '', protocols, sec_guarantee, ax[0][1], text_size_over_labels=text_size_over_labels)

#####################################################################################################

#order everything nicely -- Data online
build_graph(data_dict_online, 2, 'Data Sent\nParty 0 (MB)', protocols, sec_guarantee, ax[1][0], text_size_over_labels=text_size_over_labels)

#####################################################################################################

#order everything nicely -- Data - offline
build_graph(data_dict_offline, 2, '', protocols, sec_guarantee, ax[1][1], text_size_over_labels=text_size_over_labels)

#####################################################################################################

#order everything nicely -- Rounds - online
build_graph(rounds_dict_online, 0, 'Comm. Rounds', protocols, sec_guarantee, ax[2][0], label_type=int, text_size_over_labels=text_size_over_labels)

#####################################################################################################

#order everything nicely -- Rounds - offline
build_graph(rounds_dict_offline, 0, '', protocols, sec_guarantee, ax[2][1], label_type=int, text_size_over_labels=text_size_over_labels)
...
ax[2][0].text(0.4, -0.25, "Online", transform=ax[2][0].transAxes, ha='center', va='top', fontsize=text_size+25, color='black')
ax[2][1].text(0.55, -0.25, "Offline", transform=ax[2][1].transAxes, ha='center', va='top', fontsize=text_size+25, color='black')
...
plt.legend(ncol=2,handles=[blue_patch, red_patch],fontsize=text_size+8,loc='lower center',bbox_to_anchor=(-.1, -0.5),)
